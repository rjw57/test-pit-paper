...
\begin{abstract}
    ...
    significance based on Welch's $t$ test. For our purposes, `significant' is
    taken to mean `surprising given the wider context'. Such significant test
    pits can be used to localise settlement or other processes leading to
    larger-scale potter deposition.
\end{abstract}
...
There exists a standard formula for the degrees of freedom, $\nu^{(R)}_i$, from
...
$+\infty$ this rule is reasonable.)

For completeness, the relation for $\nu^{(R)}_i$ is.
\[
